...
app.post('/add', function (req, res) {
  ...
  database.ref('Blogs/').orderByChild('id').equalTo('${projectData.id}').on('value', function(snapshot) {
      snapshot.forEach(function(data){                      
          var val = data.val();
          var blogID = val.id;
          document.getElementById('title').innerHTML = val.Title;
          document.getElementById('date').innerHTML = val.date;
          document.getElementById('image').src = val.ImageUrl;
          document.getElementById('image').title = val.Title;
          document.getElementById('image').alt = val.Title;
...
function submitComment() {
    firebase.auth().onAuthStateChanged(function(user) {
        if (user) {
            ...
            var firebaseOrders = database.ref().child('comments/${projectData.id}/');
...
// -------------------------------------------------------------------------- //
// Display Comments
firebase.database().ref('comments/${projectData.id}/').on('value',function(comments){
            var content = "";
  ...
  `
  fs.writeFile(`public/blogs/${projectData.title}-${projectData.id}.html`, content, function (err) {
    if (err) throw err;
    console.log('Saved!');
  });
});
